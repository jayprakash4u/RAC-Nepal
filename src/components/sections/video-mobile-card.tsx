...
function VideoMobilePlayer({ video }: { video: VideoItem }) {
  ...
  if (isPlaying) {
    return (
      <div className="relative aspect-video w-full bg-navy">
        <iframe
          src={getYoutubeEmbedUrl(video.youtubeId, video.startSeconds, true)}
          title={video.title}
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-linear-to-t from-navy/80 via-navy/30 to-navy/10"
      />
      ...
      <span
        ...
        className={cn(
          ...
          "items-center justify-center rounded-full bg-white shadow-lg scale-110",
        )}
      >
        <PlayIcon />
      </span>
...
export function VideoMobileCard({ video }: { video: VideoItem }) {
  ...
  return (
    <article
      className={cn(
        ...
        "border border-slate-200/90 bg-background shadow-md",
      )}
    >
      <VideoMobilePlayer video={video} />
      ...
      <div className="flex flex-1 flex-col gap-sm px-lg py-lg">
        ...
        <a
          ...
          className="mt-auto inline-flex items-center gap-xs text-small font-medium text-primary"
        >
